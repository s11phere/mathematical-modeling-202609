...
def main():
    ...
    parser.add_argument("--dev", action="store_true", help="仅用于 smoke/full 开发实验；允许已有 Python 源码修改，仍严格核验数据和结果")
    args = parser.parse_args()
    if args.dev and not (args.smoke or args.full):
        parser.error("--dev 只能搭配 --smoke 或 --full，不能用于审计或制图。")
    ...
    verification = verify_package(dev=args.dev)
    print(json.dumps(verification, ensure_ascii=False), flush=True)
    if args.dev:
        out.mkdir(parents=True, exist_ok=True)
        (out / "development_source_changes.json").write_text(
            json.dumps(verification, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    if args.figures:
        out.mkdir(parents=True, exist_ok=True)
        render_all(out, args.font)
    else:
        runner = load_runner(development=args.dev)
        if args.audit_only:
