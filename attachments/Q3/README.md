...
`--figures` 从保存结果生成正文图、表格和 140 页完整轨迹图册（120 页主实验、20 页消融）。附带全部动作证据，图册可随时重建；制表程序只生成数字和表格，摘要文字由论文维护。制图需用 `--font /path/to/chinese-font.ttf` 指定中文字体（共享字体不随包提交以控制支撑材料体积）；审计与实验不需要字体。
