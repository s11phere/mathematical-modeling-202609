...
def load_runner(development=False):
    ...
    # Frozen records were produced on Windows. Normalize only their path
    # fields on read; the packaged records and raw action payloads stay intact.
    original_read_json = runner.read_json
    def read_json(path):
        value = original_read_json(path)
        if isinstance(value, dict) and isinstance(value.get("rows"), list):
            for row in value["rows"]:
                if isinstance(row.get("trace_file"), str):
                    row["trace_file"] = row["trace_file"].replace("\\", "/")
        return value
    runner.read_json = read_json
...
def render_all(outdir, font_path=None, include_supplement=True):
    ...
    for row in records["rows"]:
        row["trace_file"] = str((FROZEN / row["trace_file"].replace("\\", "/")).resolve())
