...
3. **变换 + 插值**：E[D] 在**刚体变换下不变**（旋转 + 平移保持距离与夹角）。
   因此真实一次检测 ``(S, θ)`` 对应的场，就是把基准图**绕原点旋转 θ、再平移到 S**：

       b = R(-θ)·(g - S)     （g 是全局网格点，b 是基准图坐标）
       E_g = E_base(b)       （双线性插值）
...
6. **归一化**：``E_norm = E_mean / E_max ∈ [0,1]``（0 = 交会几何最好，1 = 最差）。
7. **距离惩罚层**：行动要花时间，所以在归一化场之上再叠加一层**与探测器距离线性增长**
   的代价：

       score(g) = E_norm(g) + λ · |g - p_robot| / (2·R_arena)

   λ 即"距离项系数"，可调（``dist_weight``，默认 0.35；λ=1 表示"跑到靶区对面"
...
class BaseMap:
    ...
    def transformed_lookup(self, S, theta_deg, X, Y, e_max=E_MAX):
        """把基准图按"检测点 S + 示向度 θ"旋转平移后查询全局网格点 (X, Y)。

        ``b = R(-θ)·(g - S)``：把全局点转回基准图坐标系（基准图的源在 +x 轴方向）。
...
class BaseMapFamily:
    ...
    **为什么需要它（这一步不做对，整个"变换 + 插值"就不成立）**
    ...
    基准图里源位先验是"从检测点沿示向度射线、``t ∈ [5, t_hi]``、权重 ∝ t"。
    真实场景中源还必须落在靶区内，所以射线上先验的**外径** ``t_hi`` 随 ``(S, θ)``
    ...
    def build(cls, outdir, t_his=(150.0, 250.0, 350.0, 450.0, 600.0, 800.0, 1000.0,
        ...
        man = {"kind": "p3_base_map_family", "step": float(step), "n_t": int(n_t),
               ...
               "note": "每个文件是 S1=(0,0)、θ1=0 下，源位先验 t∈[5,t_hi] 的 E[D|可检测]"}
    ...
    def rebase(self, S, theta_deg):
        """给出 (S, θ) 检测对应的基准图：先按射线出靶区距离取 t_hi，再在族内插值。"""
        S = np.asarray(S, float)
...
class FieldConfig:
    ...
    dist_weight: float = 0.35       # 距离项系数 λ（可调）
...
class ExpectField:
    ...
    def add_bearing(self, S, theta_deg, weight=1.0, tag=None):
        """叠加一次检测 ``(S, θ)`` 给出的期望场（已裁剪：未覆盖 = e_max）。

        有基准图族时先用 (S, θ) 的"射线出靶区距离"选 t_hi，再在族内插值，
    ...
    def score(self, cur_xy, visited=(), extra_penalty=None, max_dist=None):
        """计算整张网格的 score = 归一化期望 + λ·距离/靶区直径（排除项 = +inf）。"""
...
def best_refine_point(rec_pts, rec_svds, cur_xy, region, cfg: FieldConfig | None = None,
    ...
    与 :class:`ExpectField`（全局、用预计算基准图）互补：这里对任意 S1/θ1 直接算
    ...
    #   清扫路程 ≈ 圆域面积 / 覆盖行距 = π·(r)²/(2·20)，r 取 E[D]/2 + 20 m
    v = 5.0
...
def _ring_candidates(S, theta_deg, radii=(400.0, 800.0, 1200.0), n_ang=8,
    ...
    """绕检测点 S 的环形候选点（相对 θ 偏开 dphi0，避免正落在退化射线上）。"""
    S = np.asarray(S, float)
    C = []
    for r in radii:
        for k in range(n_ang):
...
def selfcheck(base_csv=DEFAULT_BASE_CSV, verbose=True, seed=20260913):
    ...
    rec("T5d 距离层：score = E_norm + λ·d/(2R_arena)（λ=0 时与距离无关）",
    ...
    rec("T6b 变换+插值 ≈ 精确期望（良态几何）", ok6, detail6)
...
def field_figure(f, cur_xy, sel, path, title=None):
    ...
    panels = [(E_mean, "E[intersection diameter] (m)\n(uncovered cells = 1500)",
              ...
              (np.where(np.isfinite(info["dist_m"]),
               ...
               f"distance term  λ·d/(2R),  λ={f.cfg.dist_weight}", "magma"),
