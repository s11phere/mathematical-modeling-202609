...
真源必在
    ...
    R = ∩_i W_i,   W_i = { X : |ang(X - S_i) - θ_i| <= ε }

W_i 是以 S_i 为顶点、张角 2ε 的**角楔**，等价于两个半平面之交：

    W_i = { X : a⁻_i·X + b⁻_i <= 0 } ∩ { X : a⁺_i·X + b⁺_i <= 0 }

因此 R 是 2k 个半平面的交 —— **凸多边形**，顶点数 ≤ 2k。

算法要点（含两处易错点的正确处理）
----------------------------------
1. **可行性 / 有界性**（先判空、再判无界，顺序不能反）
   * 空集：两两示向度可以相差 > 2ε，但两个"错开"的角楔也可能没有公共点；
   * 无界：存在方向 d 使 |ang(d) - θ_i| ≤ ε 对所有 i 成立
     ⟺ 所有示向度**两两夹角 ≤ 2ε**；此时直径 = +∞，不能报有限值。
2. **区域顶点**：凸集的顶点必是两条起作用约束边界直线的交点，故枚举 2k 条边界直线的
   O(k²) 个交点、用**半平面判据** a·X + b ≤ 0 筛选即可（精确）。
...
4. **直径**：顶点数 ≤ `DIAM_BRUTE_LIMIT` 时暴力枚举；否则旋转卡壳，两者在自检中互相校验。
5. **覆盖判定**：以直径 AB 为直径的圆 C，
   V ∈ C ⟺ ∠AVB ≥ 90°（Thales）⟺ |V - c| ≤ D/2（c 为 AB 中点）。
   一般**不能覆盖**；正确做法是用**最小包围圆**（半径 r* ∈ [D/2, D/√3]，Jung 定理）。
"""
...
def region_by_clipping(pts, svds, err=BEARING_ERR, box=CLIP, tol=DIST_TOL):
    ...
    返回 (P, status)：status ∈ {"empty","degenerate","polygon","unbounded"}；
...
def region_from_bearings(pts, svds, err=BEARING_ERR):
    ...
    status ∈ {"empty", "unbounded", "degenerate", "bounded"}。
    """
...
def min_enclosing_circle(P):
    ...
    理论：r* ∈ [D/2, D/√3]（Jung 定理，平面），等号右端仅当区域为等边三角形时取到。
    """
    P = np.asarray(P, float)
    n = len(P)
    if n == 0:
