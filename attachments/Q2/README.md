...
2026-09-13 论述更正：保存结果采用“测站到整段源位距离不超过 1500 m”的门槛，没有逐源位接收筛选。本版正文将其限定为理想中心方位下的几何布点估价；历史字段、注释和补充图中的 “detectable” 不能解释为真实接收概率。详见 `DATA_SOURCES.md`。此次保留算法、网格及历史校验记录，未重跑实验。
...
远程版本此前实际执行了审计、试运行及制图，并独立复制到原项目之外重验这三项。82 个固定点
