...
def verify_package():
    ...
    for relative,record in manifest['files'].items():
        path=ROOT/relative
        assert path.is_file() and hashlib.sha256(path.read_bytes()).hexdigest()==record['sha256'],relative
    ...
    for path in (ROOT/'code').glob('*.py'):
        ...
        if key in frozen:
            assert hashlib.sha256(path.read_bytes()).hexdigest()==frozen[key],('frozen source',key)
            count+=1
    return {'package_files_verified':len(manifest['files']),'included_frozen_source_files_verified':count}
