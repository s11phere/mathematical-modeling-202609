...
本轮在 `codex/b-locator-q3` 完成问题三写作，算法及默认配置保持原样。正文为 7 页（第 15–21 页），摘要仅回填 Q3，正式测试三行待补；Q4、摘要总述和全篇总评仍为占位。按后续格式要求，各级中文标题统一采用摘要的黑体字重，Q3 小标题改为 `(a)`、`(b)` 等独占一行；全局页边距、字号和间距参数保持原样。Q1、Q2 的内容保护及本次格式检查见 `out/paper_q3/preservation_check.json`、`formatting_check.json`。
...
| 全部 120 场景、五策略补充轨迹图册 | `B_locator/Q3/figures/supplementary_trajectories.pdf` |
| 冻结逐局数据及汇总 | `B_locator/Q3/results/paired_results.csv`、`summary.json` |
| 680 份动作与退出后真值记录 | `B_locator/Q3/research_archive/out/paper_q3/traces/` |
...
后三行的文件分布在 `B_locator/Q3/results/` 和 `research_archive/out/paper_q3/`。完整算法源码在 `Q3/code/`，历史版本和审计快照在 `research_archive/src/`；论文附录仅展示最终策略核心代码。
...
python3 -B B_locator/Q3/research_archive/scripts/make_paper_tables.py
MPLCONFIGDIR=/tmp/q3-paper-mpl python3 -B B_locator/Q3/research_archive/scripts/make_paper_figures.py
MPLCONFIGDIR=/tmp/q3-paper-mpl python3 -B B_locator/Q3/research_archive/scripts/make_paper_trajectory_atlas.py
bash B_locator/paper/build.sh build

# 在新目录完整重跑 680 次，避免混入论文冻结数据。
python3 -B B_locator/Q3/reproduce.py --out /tmp/q3-paper-reproduction
```
...
```sh
python3 -B B_locator/Q3/research_archive/src/p3_selftest.py --out /tmp/q3-selftest.txt
python3 -B B_locator/Q3/research_archive/src/p3_adaptive_checks.py
python3 -B B_locator/Q3/research_archive/src/p3_joint_checks.py
```
...
| 论文内容 | 公式标签或图表 | 实现与证据 |
|---|---|---|
| 完整成本与退出时间 | `eq:q3-cost`、`fig:q3-time` | `code/p3_arena.py` 接口；`research_archive/scripts/run_p3_paper.py::action_audit` 独立重算；每局 `actions`、`action_audit` |
| 保守角楔及清除半径 | `eq:q3-wedge`、`eq:q3-region` | `code/p3_homing.py`、`code/p3_coverage.py`、`code/p3_robot.py`；专项检查 |
| 五代策略的递进与逐局例外 | `tab:q3-generations`、`fig:q3-fst`、`fig:q3-taj` | `code/p3_robot.py`、`p3_sweep.py`、`p3_tour.py`、`p3_adaptive.py`、`p3_joint.py`；固定 random/index=0/seed=2026091200 的五份 trace |
| 共用基线、逼近与侧移 | `eq:q3-baseline`、`eq:q3-home` | `code/p3_homing.py`、`code/p3_adaptive.py` |
| 联合覆盖及逐频道扫描代理 | `eq:q3-gain`、`eq:q3-greedy`、`eq:q3-plan` | `code/p3_frontier.py`、`code/p3_joint.py`；完整成本仍以动作审计为准 |
| 保留独占覆盖职责的测站移动 | `eq:q3-move-constraint`、`eq:q3-move` | `code/p3_joint.py`；`joint_no_cover_polish` 消融 |
| 连续覆盖与停机 | `eq:q3-cert-radius`、`eq:q3-cert-proof`、`eq:q3-stop` | `code/p3_coverage.py`；独立 `CoverageAuditor`、`coverage_audit.json` |
| 可选补测中心射线近似 | `eq:q3-prob` | `code/p3_joint.py`；`joint_no_probability_gate` 消融与强制目标专项检查 |
