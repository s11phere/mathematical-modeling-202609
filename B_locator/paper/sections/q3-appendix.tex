...
贪心覆盖 / 规划候选 & 最多 30 次；$(\alpha,\beta)$ 取 $(1,0.3)$、$(1,1)$、$(2,0.3)$、$(2,1)$、$(3,0.3)$\\
...
实验数据与逐动作记录保存在 \texttt{B\_locator/Q3/out/paper\_q3/}，附有代码、期望场及场景参数的校验信息。图表从同一组数据生成，完整复现命令如下。
\begin{lstlisting}[language=bash,caption={从仓库根目录复现问题三}]
python B_locator/Q3/scripts/run_p3_paper.py
python B_locator/Q3/scripts/make_paper_tables.py
python B_locator/Q3/scripts/make_paper_figures.py
python B_locator/Q3/scripts/make_paper_trajectory_atlas.py
python B_locator/Q3/scripts/make_paper_source.py
python B_locator/Q3/src/p3_selftest.py --out /tmp/q3-selftest.txt
...
环境与验证说明见 \path{Q3/PAPER_REPRODUCE.md}；全部 120 组五策略轨迹见输出目录中的 \path{supplementary_trajectories.pdf}。

\clearpage
\section{问题三完整源程序}\label{app:q3code}
以下列出五代策略及实验所需的完整模块，可执行源文件与校验值见支撑材料。性能比较以正文的统一实验为准。
\input{sections/q3-source-listings.tex}
