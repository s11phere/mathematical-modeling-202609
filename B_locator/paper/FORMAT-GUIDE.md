...
- ✅ **强调中文一律用摘要同款黑体**：优先写 `{\heiti 关键词}`；花括号已将字体声明
  限定在局部，括号外自动恢复原字体。现有 `{\heiti 关键词}\songti` 写法也可保留。
- `\textbf{中文}`、`\bfseries` 的中文部分已统一映射到同一黑体；黑体粗体与常规字重
  使用同一字体文件，不再叠加 `AutoFakeBold`。西文字母、数字仍遵循 Times 的粗体设置。
- ❌ 不要写 `\heiti{文字}`：`\heiti` 是声明式命令，不能用参数花括号限定作用域。
...
| 一/二/三级标题 | 摘要同款黑体，不叠加仿粗；编号数字保留 Times 粗体 |
| 论文标题、「摘要」二字 | 摘要同款黑体，不叠加仿粗 |
| 中文粗体 | 所有 CJK 家族的粗体/粗斜体均映射到上述黑体的常规字重 |
