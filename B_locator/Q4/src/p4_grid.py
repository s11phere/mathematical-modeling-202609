...
三个定义（都被后面的数值报告使用）
----------------------------------
1. **覆盖半径** ``cov(p)`` = p 到最近停靠点的距离。源的接收半径 ≥ 1000 m，
   所以 ``cov(p) > 1000`` ⇒ p 处若有源，任何停靠点都听不到它。
2. **定向覆盖半径** ``dir_cov(p)`` = p 到最近**可被听到**停靠点的距离：

       q 能听到 p 处的源  ⟺  |q-p| ≤ 1000  且  (q-p)·d ≥ 0

   其中 ``d`` 是源的定向方向（180° 覆盖角）。后一个条件等价于
   "q 落在以 p 为界、朝向 d 的那个闭半平面里"。
3. **顶点三向覆盖（用户的直觉）**：若 p 落在三角形 v1v2v3 内部，则
   ``p = Σλᵢvᵢ (λᵢ ≥ 0)``，于是对任意方向 d 有 ``Σλᵢ(vᵢ-p)·d = 0``
   ⇒ 必有某个 i 使 ``(vᵢ-p)·d ≥ 0``。**换句话说：三角形的三个顶点方向
   正向张成整个平面，任何定向方向都至少被一个顶点"看到"。**
   所以只要再保证 ``max_i |vᵢ-p| ≤ 1000``（等边三角形内的上界为边长 a），
   这个三角形内的任何源（不论朝向）都必被某个顶点听到。

推论：完整三角剖分且边长 a ≤ 1000 m 是充分条件；还必须覆盖靶区边界。
a/√3 仅表示最近格点距离，不能代替最远顶点距离。下方离散评估只作诊断；
...
# 三角形格网的基向量（边长为 a 的等边三角形）：u = (a,0)，v = (a/2, a√3/2)
SQRT3 = math.sqrt(3.0)
...
def hex_ring_lattice(a, rings=2, origin=(0.0, 0.0)):
    """"同心六边形环"形式的格点：第 k 环半径 = k·a，含 6k 个点。

    与 :func:`triangular_lattice` 是同一个格网，只是**按环分组**——
...
def cov_stats(pts, gx=None, gy=None, r_cover=R_RECV_MIN):
    ...
    完备性判据：``最大未覆盖距离 <= 1000 m`` ⇒ 任一源（不论朝向）都会被某个
    停靠点听到 —— 对**全向**源严格成立；对定向源还需要半平面条件，见
    :func:`dir_cov_stats`。
    """
    if gx is None:
        gx, gy = eval_grid()
...
def dir_cov_stats(pts, gx=None, gy=None, r_cover=R_RECV_MIN, r_arena=R_ARENA,
    ...
    对给定方向 d，"能被听到"要求存在 q 同时满足距离与半平面条件，所以
        ...
        dir_cov(p, d) = min{ |q-p| : |q-p| ≤ r_cover, (q-p)·d ≥ 0 }
