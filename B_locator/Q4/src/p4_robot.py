...
class P4Config(P3Config):
    ...
    # ---- 格网 ----
    grid_a: float = 1000.0          # 三角形格网边长（m）：a/√3 = 577 m 覆盖半径
    grid_r_max: float = R_ARENA     # 格点铺到多大半径（靶区半径 1800 m）
    outer_rings: tuple = ((1850.0, 12),)
    """外侧补漏环 ``(半径 m, 点数)``：救"贴边朝内发射"的定向源。

    ⚠️ 先说清这个环到底在救什么（实测修正了我最初的直觉）：
    mock 的定向判据是"站→源 的视线落在 ``dir_deg`` 的 ±90° 内"，
    诊断 100 局漏源（`src/p4_miss_where.py`）显示漏掉的全是
    **半径 1622–1794 m、朝向与径向夹角 117°–178°（几乎正对圆心）的定向源**。
    也就是说：**"朝内发射"的源必须从它外侧的站位才听得到** —— 外环就是干这个的。

    靶区边界附近（r ≳ 1000）的源若朝内发射，覆盖半平面里没有任何"域内"站位，
    ...
    | r1900×6（≈"格网整体铺到 r=2000"） | 19 | 575 m | 1.91% |
    | r1850×9 | 22 | 639 m | 2.59% |
    ...
    ring_band_frac: float = 0.5     # 螺旋航路的"环"分档宽度 = grid_a × 该比例（m）。
                                    # 0.5 → r=1732 与 1850 合成一层（实测最好，航程 26.3 km）；
                                    # 0.35/0.25/0.18 → 分成 3 层，反而变差（28.0 km、长跳 +1）
    ...
    loc_cross_radius_m: float = 420.0  # 侧移交会的离源距离（m）：交会角 ≈45°
    ...
    clear_detour_max_m: float = 300.0  # **阶段式**（`unified=False`）下"到站顺手清"的绕行上限。
                                    # ⚠️ 统一航路**不用**这个值（用 interleave_detour_max_m，
                                    # 默认 ∞）。曾经误把 300 m 套到统一航路上，结果把大多数
                                    # 清理延后、积压到航程末尾：平均定位清除 648 → 1141 s、
                                    # 行程 34.9 → 64.1 km。两套策略的参数不能混用。
...
class P4GridRobot(P3Robot):
    ...
    def __init__(self, arena, cfg: P4Config | None = None, base=None, log=None,
        ...
        self._near_pt: dict[int, tuple] = {}    # "距离 ≤ 5 m" 的检测点：到那儿直接清
    ...
    def covering_set(self):
        ...
        为什么需要它：覆盖判据（:meth:`coverable_mask`）是"站点 × 格点"的集合覆盖
        问题，30 个站里往往只有十几个真正贡献新覆盖。把"必须去的站"算出来，
        航线才不会为了几个格点多跑几公里。

        贪心在这里是**安全**的：它只决定"访问顺序"，不改变覆盖判据 ——
    ...
    def measure_set(self):
        ...
        为什么这是一个独立的省时杠杆：时间 ≈ Σ_站 (移动 + 该站要测的频道数 × 6 s)。
        "访问哪些站"由覆盖判据决定（全测一遍才敢说"没有残留源"），
        但如果允许**放弃一部分区域**，就可以只保留一个"代价最小、覆盖大部分"的站集：

        ``cover_reduce = r`` ⇒ 用贪心集合覆盖，一直取到"剩余未覆盖格点
        只剩全部格点的 r 倍"为止 —— 于是**故意留 r 比例的面积不测**。

        这是"覆盖 → 用时"最直接的旋钮：r=0 就是原来的全覆盖行为。
        """
    ...
    def plan_station_tour(self, start=None):
        ...
        **"早点听到源"比"少走路"更值钱** —— 源一旦被听到就会立刻进入清理队列
        （实测：纯最短航路让某一局的 14 个源晚了 2000 s 才被听到，指标掉 2 个百分点）。
        所以在距离代价上加 ``spiral_radius_w · 半径``：航路自然由内向外展开，
        同时 2-opt 仍能消掉最明显的绕远。

        ``start`` 给定时以它为起点重排（滚动重规划用）；默认从原点出发。
        排序只花几十毫秒，却能把扫描段从 24 km 压到 16–18 km。
        """
    ...
    def _grid_pts(self):
        ...
        分辨率不能太粗：覆盖判据是"格点到站点的距离 ≤ 有效接收半径下界 1000 m"，
        而三角格网的覆盖半径已经接近这个量级（577 m），用 100 m 粗网格会让
        边界附近的源被漏判（实测有源因为格点间隙而永远没被听到 —— 这是**真的漏源**，
    ...
    def coverable_mask(self, si, flip=None):
        ...
        半平面条件（以"源在 g、记发射方向为 d"为准）：站位 ``q`` 能听到 ⟺
        ``|q-g| <= 1000`` 且 ``(q-g)·d >= 0``（视线方向落在 ±90° 覆盖角内）。

        本题只给了"定向方向未知、覆盖角 180°"，**没有**规定 d 的指向约定
        （是"朝 d 发射"还是"d 的反方向发射"）。两种约定给出的可听半平面正好相反：

        * ``flip=False``：d = 径向向外 ``g`` ⇒ 要求在**外侧**（``(q-g)·g >= 0``）；
        * ``flip=True`` ：d = 径向向内 ``-g`` ⇒ 要求在**内侧**（``(q-g)·g <= 0``）。

        为了让策略对协议约定**不敏感**，扫描阶段做**两遍**：第一遍按 ``flip=False``
        覆盖，若有频道仍然没被听到，第二遍按 ``flip=True`` 把另一半半平面也扫掉。
        两遍合起来 ⇒ 无论真实约定是哪一种（甚至两向都发），源都必被听到
        ...
        d2 = (gx - qx) ** 2 + (gy - qy) ** 2
        dot = (qx - gx) * gx + (qy - gy) * gy       # (q-g)·g：q 在源的外侧
        if flip:
            dot = -dot
    ...
    def unheard_cover_left(self):
        """所有未听频道剩余未覆盖格点的总量（0 ⇒ 可以断定靶区内没有别的源了）。"""
    ...
    def pick_station(self):
        ...
        ``score = 新增覆盖格点数 / (1 + 行驶距离/500) - w·半径/100``

        * 分子是"这一站还能补多少我们没听过的地方"（用覆盖判据算，见
          :meth:`coverable_mask`）；分子为 0 的站直接淘汰 —— 这是"该收工就收工"
    ...
    def station_channels(self, si, pos=None):
        ...
        已经听到过的频道一律不再在这里测 —— 定位由清理阶段的专用测点负责
        （实测教训：在扫描途中反复测同一个已发现频道，会把它带离定位几何、
        也把航程和虚拟时间白白放大一倍以上）。

        ``cover_reduce > 0``（降覆盖档）时还要再过滤一层：这一站必须**在
        :meth:`measure_set` 里**才测 —— 不在集合里的站，走到也不测（省下 6 s/频道）。
        """
    ...
    def locate_on_ray(self, ch):
        ...
        * 沿射线往外走，信号迟早消失：要么是距离超过接收半径，要么是走出了
          ±90° 覆盖角 —— 两种情况都意味着"源在这条视线上的位置就在消失点附近"，
        ...
        * 于是"有信号/无信号"的分界点就能把源夹住，二分几步即可收到 40 m 以内。
        ...
        # 向外用 1.4 倍递增的步长跳（520 → 730 → 1020 …），边界通常在 2–3 步内被夹住。
    ...
    def locate_by_cross(self, ch):
        ...
        ``loc_cross_radius_m`` 处 —— 交会角 ≈ 45°、基线 ≈ 400 m
        ⇒ 区域半径 ≈ R·tan1°/sin45° ≈ 10 m，覆盖式试探一次就中。
    ...
    def solve_channel(self, ch):
        ...
        if rec.cleared:
            return True
        # (0a) 定位过程中出现过 near（距离 ≤ 5 m）：回那个点直接清，别再做任何试探
        ...
        for _ in range(self.cfg.clear_approach_max):
            ...
            if resp.get("accepted") is not True:
                return False
        # (3) 第一轮试探：有界区域（且不大）就按**交会多边形**试 —— 交会区域常是细长
        #     四边形，按多边形只在"到区域 ≤ 20 m"的格点上试，比外接圆省一半以上的点，
        ...
        #     定向源的清除与朝向无关 ⇒ 只要"估计点到真源 ≤ 20 m"就够了；
        #     沿视线二分的距离精度约 ±20 m，横向误差约 0.0175·d ⇒ 半径 90 m 足够。
        if est is not None:
        ...
        if rec.n_bearings >= 1:
            ...
            if br is None:
                ...
                if est2 is not None and self.sweep_ok(
                        ...
                        self.cfg.loc_clear_radius_m, poly=None):
                    return True
        # (5) 第四轮：**沿视线二分给出的是"与协议约定无关"的距离信息** ——
        #     只要二分区间收得够窄（≤ 230 m，说明测点确实落在源的覆盖范围内），
        #     就以该位置为中心做一次小圆试探。这一轮专门对付"几条视线近似平行
        #     （交会角小）、有界区域又细又长"的目标 —— 那类目标的交会区域虽然把源
    ...
    def sweep_pass(self, flip=False):
        ...
        * 该半平面约定下"还没有被排除的格点"清零 ⇒ 这一遍已经证明没有漏源；
        * 航线走完；
        * 时间不够。
    ...
    def scan_skeleton(self, start=None):
        ...
        旧版按"半径分档 + 档内极角"排成**固定**顺序 —— 进环永远从固定角度切进去，
        于是"切换环"时经常要横穿靶区。实测骨架直线航程 18.7 km，
        但跳站之后实际扫描段走了 **26.2 km**（多出来的就是跨环长跳）。

        规则（对应用户的要求：**切换环时就近进入**）：

        1. **全局最近邻**（不分环）：下一个站取"离我最近的那个" ——
           "进环点"自然就是最近的站，而不是某个固定角度；
        2. 代价里加 ``w · 半径`` 的小偏置：让航线**整体**由内向外推进
           （内环先扫 ⇒ 中心附近的源先被听到，对指标有利），
           但偏置足够小，不会为了"先内后外"去横穿靶区；
        3. **2-opt** 消掉自交与绕远。

        ⚠️ ``start`` 只用于"排一条**基准航线**"，不要拿"顺路清除之后的当前位置"来排
        —— 从偏离航线很远的点做最近邻会得到一条"以该点为中心的星形路线"，
        ...
        # 动机是"先扫内环 ⇒ 中心附近的源早被听到 ⇒ 指标更好"，
        # 但实测（20 局）：gain=180 时从 620 s 变 625 s（更差）；
        # gain=250 时从 602 s 变 578 s（更快）**但清除率 1.0000 → 0.9925**（掉 2 个源）。
    ...
    def spiral_tour(self, start=None):
        ...
        单调前进"，`adv ≥ 0.35 格`）搬到问题 4 的格网上：

        1. 环 = 半径分档（``grid_a/2``，于是 r=1000 / 1732 / 1850 各成一层）；
        2. **由内向外**逐环扫（内环先扫 ⇒ 中心附近的源先被听到，指标对"听到时刻"敏感）；
        3. 每环内**角向单调**：方向在第一环的第一段确定（取"往哪边转更近"），
           之后只沿这个方向走 —— 绝对不会在同一环上回头；
        ...
        1000,1850,1732,1000,…），几何上更短，但**角向来回横跳**；
    ...
    def build_queue(self, start=None):
        ...
        * 旧版 A：每一步在"还没去过且此刻有用的站"里取最近 ——
          会跳过"此刻收益为 0"的几何中间节点，出现 2 500 m 的长跳，之后还要回头补；
        * 旧版 B：顺路清除后按当前位置重排剩余站位 ——
        ...
        * ``"tsp"``（默认）：:meth:`scan_skeleton`（全局 NN + 2-opt + cheapest-insertion
          多起点取优）—— 几何最短，但会在环之间交替穿插；
        * ``"spiral"``：:meth:`spiral_tour` —— 问题 3 式"由内向外绕圈 + 角向单调不回头"。
    ...
    def next_scan_station(self, flip=None):
        ...
        **两步走**（这是把"就近选站"和"不跨环乱跳"同时做到的办法）：
        ...
        2. **被顺路清除带偏之后**（当前位置到队首的距离 > ``detour_follow_m``）：
           不再硬走回队首，改为**就近切入**——在基准航线里挑"离我最近的那一站"
           作为新的起点，然后**顺着基准航线的顺序**往下走（不是重新做最近邻！）。

        为什么"顺着基准航线"而不是"重新最近邻"：重新最近邻会从偏离点长出一颗
        **星形路线**，把环形结构彻底打散（实测航程 17.9 km → 26.9 km），
    ...
    def localize_fast(self, ch):
        ...
        1. 交会区域已经 ≤ ``interleave_loc_radius_m`` ⇒ 直接用（0 次额外测量）；
        2. 侧移交会一次（420 m、交会角 ≈45°）—— 一步就把区域压到 10–40 m，
    ...
    def clear_here(self, ch):
        """在当前附近把一个已听到的目标清掉：小幅度定位 → 小范围覆盖式试探。
        ...
        if est is None:
            return False
        # 顺带：人已经在估计点上/附近，直接试一次 /clear（3–5 s 的事）
    ...
    def pick_unified_action(self, attempts):
        ...
        `src/p4_tune_unified.py` 的扫参（20 局）把"绕去清理的距离门限"从 0 扫到 ∞：
        ...
        | **∞（听到就清）** | **1.0000** | **833 s** | **11 162 s** | **45.9 km** |

        门限越松越好、一直放到"不设门限"最优 —— 原因是**流水线**：
        ...
        for ch in self.heard_pending():
            ...
            # 绕行上限用 ``interleave_detour_max_m``（默认 ∞ = 不设限）。
            # ⚠️ 别用 ``clear_detour_max_m``：那是阶段式时代留给"到站顺手清"的 300 m，
            # 套到统一航路上会把大多数清理都延后 ⇒ 积压到航程末尾、来回折返
            # （实测 300 m 上限：648 s → 1141 s、行程 34.9 → 64.1 km）。
    ...
    def run_unified(self):
        ...
        * 收工判据不变：两遍都走完且所有目标已清 ⇒ 靶区内没有未发现的源。
        """
        n_clear = 0
        ...
        for pi, flip in enumerate(passes):
            ...
            if pi > 0 and not self.unheard() and not self.heard_pending():
                break
            self.log(f"—— 第 {pi + 1} 遍（半平面约定 "
                     f"{'朝内' if flip else '朝外'}）："
                     f"未听频道 {len(self.unheard())}，待清目标 "
                     f"{len(self.heard_pending())} ——")
    ...
    def _staged_scan_then_clear(self):
        ...
        ``P4Config(unified=False)`` 时使用 —— 保留它是为了做"边扫边清 vs
    ...
    def _opportunistic_clear(self, radius_m=None, max_n=2):
        ...
        为什么重要（路径图直接暴露的问题）：平均定位清除时间 = 定位清除总时刻 ÷ 清除个数，
        ...
        * 绕行 = 从当前位置到目标再折回航线的距离；只有 ``dist ≤ clear_detour_max_m``
          才值得绕（否则等航线自然路过更省）；
        * 估计位置来自"沿视线二分"（与协议约定无关），精度 ~20–40 m，
    ...
    def clear_all_heard(self):
        """清理阶段：滚动时域——每清一个就按最新位置重排剩余目标。

        三条防呆（都是实测出来的教训）：

        * 同一频道最多进 :meth:`solve_channel` ``max_solve_attempts`` 次
          （否则"定位→试探→失败→再定位"会在同一个点上无限循环，实测烧掉 30 000 s）；
        * 剩余时间不够就退出，不做无谓动作；
