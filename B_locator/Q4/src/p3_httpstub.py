...
用法（终端 A 起桩，终端 B 跑真实入口）::
    ...
    python B_locator/Q4/src/p3_httpstub.py --seed 20260913 --port 2027 --delay-open 8
    python B_locator/Q4/src/p4_run.py --mode live --url http://127.0.0.1:2027 --robot-id TEST0001 \
...
def _log(rec):
    ...
    every = int(STATE.get("every") or 1)
    if every <= 1 or STATE["n"] % every == 0 or rec.get("path") in ("/enter", "/exit"):
        print(f"  [stub #{STATE['n']}] {rec.get('path')} -> "
              f"{json.dumps(rec.get('resp', {}), ensure_ascii=False)[:150]}", flush=True)
...
def serve(arena, port=2027, real_budget=1200.0, log="", delay_open=0.0, every=1,
          open_hint="现在可以运行 p3_run.py --mode live 了"):
    """把任意 `MockArena` 按官方协议挂到 127.0.0.1:<port>（问题 3/4 共用）。"""
    STATE["arena"] = arena
    STATE["real_budget"] = float(real_budget)
    STATE["log"] = log
    STATE["every"] = int(every)
    if delay_open > 0:
        print(f"接口 {delay_open:.0f} s 后才开放（复现倒计时期间连接被拒）...", flush=True)
        time.sleep(delay_open)
    srv = ThreadingHTTPServer(("127.0.0.1", port), Handler)
    srv.daemon_threads = True
    print(f"监听 http://127.0.0.1:{port} —— {open_hint}", flush=True)
    try:
        srv.serve_forever()
...
def main():
    ...
    print("提示：这是本地桩，不是官方模拟器；案例真值仅用于写这份提示。")
