...
现行论文以 `compact + strict` 为最终策略，旧格网实现作为演进对照。正文见 `../paper/sections/q4-model.tex`，数据、公式和算法依据见 [PAPER_REPRODUCE.md](PAPER_REPRODUCE.md)。本轮重新运行 230 次配对实验，最终严格版在 60 个场景中清除全部 779 源并取得连续证书；随机主集完整退出时间 5819.1 s，较上一版严格策略降低 11.25%。五幅插图、公式推导、消融与源码附录已接入论文。
